...
class ResultParser:	
	...
	def traverseAllResult(self):
		resultPath = self.resultPath		
		if os.path.exists(resultPath):
			paths = os.listdir(resultPath)
			for path in paths:
				filePath = os.path.join(resultPath, path)                
				print filePath
				if os.path.isfile(filePath) and path.startswith('junit-report'):
					self.parseReportXml(filePath)
